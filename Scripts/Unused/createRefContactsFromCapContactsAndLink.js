function createRefContactsFromCapContactsAndLink(pCapId, contactTypeArray, ignoreAttributeArray, replaceCapContact, overwriteRefContact, refContactExists)
	...
	for (var i in c)
	   ...
	   logDebug("RefConNum = " + refContactNum);
	   
	   if (refContactNum)  // This is a reference contact.   Lets refresh or overwrite as requested in parms.
	   	{
	   	if (overwriteRefContact)
			...
			if (!r.getSuccess()) 
				logDebug("WARNING: couldnt refresh reference people : " + r.getErrorMessage()); 
			else
	   	...
	   	else  // user entered the contact freehand.   Lets create or link to ref contact.
	   	{
			...
			if (existingContact)  // we found a match with our custom function.  Use this one.
			...
			else  // did not find a match, lets create one
				{
				...
				if (!r.getSuccess())
					{logDebug("WARNING: couldnt create reference people : " + r.getErrorMessage()); continue; }
